...
public:
  explicit PrintOpLowering(mlir::MLIRContext *context)
      : mlir::ConversionPattern(quay::PrintOp::getOperationName(), 1,
                                context) {}
  ...
  matchAndRewrite(mlir::Operation *op, mlir::ArrayRef<mlir::Value> operands,
    ...
    auto printOp = mlir::cast<quay::PrintOp>(op);
    auto elementLoad =
...
namespace {
class QuayToLLVMLoweringPass
    : public mlir::PassWrapper<QuayToLLVMLoweringPass,
                               mlir::OperationPass<mlir::ModuleOp>> {
public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(QuayToLLVMLoweringPass)
...
void QuayToLLVMLoweringPass::runOnOperation() {
  ...
  patterns.add<quay::PrintOpLowering>(&getContext());
...
std::unique_ptr<mlir::Pass> quay::createLowerToLLVMPass() {
  return std::make_unique<QuayToLLVMLoweringPass>();
}
