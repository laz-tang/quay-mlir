...
class ConstantOpLowering : public mlir::OpRewritePattern<quay::ConstantOp> {
  using OpRewritePattern<quay::ConstantOp>::OpRewritePattern;

  mlir::LogicalResult
  matchAndRewrite(quay::ConstantOp op,
...
class PrintOpLowering : public mlir::OpConversionPattern<quay::PrintOp> {
  using OpConversionPattern<quay::PrintOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(quay::PrintOp op, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const final {
    // We don't lower "quay.print" in this pass, but we need to update its
    // operands.
...
namespace {
class QuayToAffineLowerPass
    : public mlir::PassWrapper<QuayToAffineLowerPass,
                               mlir::OperationPass<mlir::ModuleOp>> {
public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(QuayToAffineLowerPass)
...
void QuayToAffineLowerPass::runOnOperation() {
  mlir::ConversionTarget target(getContext());

  target.addIllegalDialect<quay::QuayDialect>();
  target.addLegalDialect<mlir::affine::AffineDialect, mlir::BuiltinDialect,
                         mlir::func::FuncDialect, mlir::arith::ArithDialect,
                         mlir::memref::MemRefDialect>();
  target.addDynamicallyLegalOp<quay::PrintOp>([](quay::PrintOp op) {
...
std::unique_ptr<mlir::Pass> quay::createLowerToAffinePass() {
  return std::make_unique<QuayToAffineLowerPass>();
}
